"""/api/cases/<name>/history — time-history slices for the plot workspace.

Batched over rows and columns for one (case, group); several cases or groups
are separate requests, since each is its own load() call (§8 of the plan).
"""
...
bp = Blueprint('history', __name__, url_prefix='/api/cases')
...
def history(name):
    ...
    for col in columns:
        ...
        for row in rows:
            series.append({'row': row, 'column': col, 'values': arr[:, row, comp].tolist()})

    return jsonify({'case': name, 'group': group, 'times': meta.times.tolist(), 'series': series})
