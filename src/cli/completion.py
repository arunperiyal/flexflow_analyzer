            ...
            if [[ -z "$subcommand" ]]; then
                ...
                case "$subcommand" in
                    ...
                    convert)
                        ...
                        ;;
                    render)
                        local flags="iso slice --vtu --config --camera --pick-camera --write-template --timestep --zone --nen --color --color-range --t1 --t2 --freq --no-vtp --output --contour --values --normal --origin --slices -v --verbose -h --help"
                        if [[ "$cur" == -* ]]; then
                            COMPREPLY=( $(compgen -W "$flags" -- "$cur") )
                        else
                            case "$prev" in
