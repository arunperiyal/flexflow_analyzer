    ...
    _SUBCMD_FLAGS: Dict[tuple, Dict[str, str]] = {
        ...
        ('field', 'compute'): {
            **_COMMON_FLAGS,
            ...
            '--sectional': 'force_coeff / separation: cut the body into N spanwise sections',
            '--azimuthal': 'separation: bins around the section (default 36 = 10 deg)',
            '--body':      "separation: whose wall_shear tables to reduce",
        ...
        ('run', 'post'):     {
            **_COMMON_FLAGS,
            '--dry-run':      'Preview without submitting',
            '--show':         'Display script contents',
            '--start':        'Process from this timestep (default: 0)',
        ...
        ('run', 'sq'):       {
            ...
            '--lines':  'Number of lines to print with --out',
            '-n':       'Number of lines to print with --out',
            '--help':   'Show help message',
            '-h':       'Show help message',
        },
        ('run', 'sb'):       {
        ...
        ('plot', None):    {
            **_COMMON_FLAGS,
            '--input-file':   'Input YAML configuration file',
            '--node':         'Node ID to plot',
            '--data-type':    'Data type: displacement or force',
            '--plot-type':    'Plot type: time, fft, traj2d, traj3d',
            '--component':    'Component(s) to plot: x, y, z',
            '--start-time':   'Start time for plot',
            '--end-time':     'End time for plot',
            '--plot-style':   'Plot style: color,width,linestyle,marker',
            '--title':        'Plot title: text|fontsize|latex',
            '--xlabel':       'X-axis label: text|fontsize|latex',
            '--ylabel':       'Y-axis label: text|fontsize|latex',
            '--legend':       'Legend label: text|fontsize|latex',
            '--legend-style': 'Legend style: position|fontsize|frameon|latex',
            '--fontsize':     'Global font size (default: 12)',
            '--fontname':     'Font family (default: sans-serif)',
            '--output':       'Output file path (.png, .pdf, .svg, .eps)',
            '--gnu':          'Display plot in terminal (gnuplot-style)',
...
class InteractiveShell:
    ...
    def handle_shell_command(self, command: str) -> bool:
        ...
        if cmd == 'ls':
            ...
            if '--help' in flags or '-h' in flags:
                ...
                self.console.print("  -t          Sort by modification time (newest first)")
                ...
                self.console.print("  ls -lt")
                self.console.print("  ls -lv RUN_1/")
                self.console.print("  ls -t *.plt")
            else:
        ...
        try:
            ...
            for path_str in expanded_paths:
                ...
                if target.is_dir():
                    ...
                    if matched_files:
                        self._list_matched_files(matched_files, long_format, natural_sort, sort_by_time)
                        matched_files = []
                    self._list_dir_contents(target, show_all, long_format, natural_sort, sort_by_time)
                else:
    ...
    def _list_dir_contents(self, directory: Path, show_all: bool, long_format: bool,
                           natural_sort: bool = False, sort_by_time: bool = False) -> None:
        """
        ...
        Args:
            ...
            sort_by_time: Sort by modification time, newest first (-t)
        """
        try:
            if sort_by_time:
                items = sorted(directory.iterdir(), key=lambda x: x.stat().st_mtime, reverse=True)
            elif natural_sort:
    ...
    def use_case(self, case_input: str) -> bool:
        ...
        Supports:
        - "Case001" - single case
        - "*" - all cases (from .cases file); commands handle iteration internally

        Args:
            case_input: Case name, path, or "*" for all cases
        """
        try:
            # Support "*" for all cases - just set it as the case name
            if case_input == "*":
                self._current_case = "*"
